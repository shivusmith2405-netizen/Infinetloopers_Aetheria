...
function App() {
  ...
  const fetchCommits = async () => {
    ...
    try {
      ...
      if (!commitsRes.ok) {
        ...
        else throw new Error(errorData.message || `Failed to fetch commits: ${commitsRes.statusText}`);
      }
  ...
  if (commitData.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#030508] px-4 relative overflow-hidden text-slate-100">
        <Starfield />

        <div className="relative z-10 w-full max-w-2xl p-10 rounded-[3rem] bg-white/5 backdrop-blur-3xl border border-white/10 shadow-[0_0_80px_rgba(168,85,247,0.15)] transition-transform duration-700 hover:scale-[1.01] hover:shadow-[0_0_120px_rgba(168,85,247,0.25)]">
          
          <div className="flex flex-col items-center">
            
            <div className="w-24 h-24 mb-8 rounded-3xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center shadow-[0_0_40px_rgba(168,85,247,0.3)] border border-white/20 relative overflow-hidden group">
            ...
            <h1 className="text-5xl sm:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400 tracking-tight mb-3 text-center drop-shadow-sm">
              AETHERIA
            </h1>
            <p className="text-slate-300 text-xl text-center mb-12 font-medium tracking-wide">
              Aetheria Code Mapper
            </p>
  ...
  return (
    <div className="min-h-screen bg-[#030508] text-slate-100 p-8 relative overflow-hidden">
      <Starfield />

      <div className="relative z-10 max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4 bg-white/5 p-6 rounded-3xl border border-white/10 backdrop-blur-xl shadow-lg">
          <div>
            <h2 className="text-3xl font-bold text-slate-100 flex items-center gap-3">
              <Map className="w-8 h-8 text-purple-400" />
              Aetheria: {repoUrl.split('/').pop()}
            </h2>
            <p className="text-slate-400 mt-2 font-medium">Displaying chronological branch history across {commitData.length} records.</p>
          </div>
        ...
        <div className="w-full h-[600px] bg-white/60 rounded-3xl border border-amber-600/20 relative shadow-[0_0_80px_rgba(245,158,11,0.2)] mt-6 backdrop-blur-xl overflow-hidden">
          ...
          {timelineCommits.length === 0 ? (
            ...
            <svg viewBox="0 0 1000 600" className="w-full h-full relative z-10 overflow-visible">
              ...
              {selectedCommit && timelineCommits.length > 0 && selectedCommit !== timelineCommits[0] && (
                (() => {
                  const fractionX = timelineCommits.indexOf(selectedCommit) / Math.max(1, timelineCommits.length - 1);
                  const startX = 100 + fractionX * 800;
                  const startY = 150 + ((parseInt(selectedCommit.sha.substring(0, 5), 16) % 1000) / 1000) * 300;
                  
                  const endX = 100;
                  const endY = 150 + ((parseInt(timelineCommits[0].sha.substring(0, 5), 16) % 1000) / 1000) * 300;
                  
                  return (
                    <g className="animate-pulse cursor-pointer">
                      <line x1={startX} y1={startY} x2={endX} y2={endY} stroke="#2DD4BF" strokeWidth="4" strokeDasharray="10,10" className="opacity-80" />
                      <circle cx={endX} cy={endY} r="15" fill="none" stroke="#2DD4BF" strokeWidth="3" className="animate-ping" />
                      <text x={(startX + endX)/2} y={(startY + endY)/2 - 10} fill="#2DD4BF" fontSize="12" fontWeight="bold" textAnchor="middle">Recommended Decision</text>
                    </g>
                  )
                })()
              )}
                  ...
                  >
                    ...
                    {isHovered && (
                      <foreignObject x={x - 175} y={y + 35} width="350" height={stellarPulse && commit.smellData ? "300" : "200"} className="pointer-events-none overflow-visible z-50">
                        <div xmlns="http://www.w3.org/1999/xhtml" className="bg-white/5 border border-white/10 p-5 rounded-2xl shadow-[0_0_50px_rgba(168,85,247,0.15)] backdrop-blur-xl text-white">
                          ...
                          <p className="text-slate-100 text-base font-bold leading-relaxed mb-2">
                            {commit.summarizedMessage}
                          </p>
                          <p className="text-slate-300 text-xs leading-relaxed overflow-hidden text-ellipsis line-clamp-2">
      ...
      {showReleaseNotes && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl animate-in fade-in duration-500">
           <div className="bg-white/5 backdrop-blur-2xl p-8 rounded-[2.5rem] max-w-4xl w-full max-h-[85vh] flex flex-col shadow-[0_0_200px_rgba(168,85,247,0.25)] border border-white/10">
               <h2 className="text-3xl font-black text-purple-400 mb-6 flex items-center gap-3 tracking-tight">
      ...
      {commitData.length > 0 && (
        <div className="fixed bottom-8 right-8 z-40 flex flex-col items-end gap-4">
           {showDeepSpace && (
             <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-6 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.6)] w-80 animate-in slide-in-from-bottom-5 duration-300">
